...
describe("BribeManagerPCS", function () {
  async function deployFixture() {
    ...
    const depositor = await ethers.getContractFactory("PCSDepositor");
    const depositorInstance = await depositor.deploy();
    await depositorInstance.initialize();
    ...
    const baseRewardPool = await ethers.getContractFactory("BaseRewardPoolV2");
    const qCakeRewardPool = await baseRewardPool.deploy();
    await qCakeRewardPool.initialize(depositorInstance.address);

    await depositorInstance.setParams(
      cake.address,
    ...
    // set whitelist vecake for voterProxy
    await voterProxyInstance.setWhitelist(true);

    await qCakeInstance.setOperator(depositorInstance.address, true);
