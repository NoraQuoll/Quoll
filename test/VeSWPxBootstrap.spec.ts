...
describe("VeSWPxBoostrap", function () {
    async function deployFixture() {
        const VeSWPxBoostrap = await ethers.getContractFactory("VeSWPxBoostrap");
        const veSWPxBoostrap = await VeSWPxBoostrap.deploy();
        return {veSWPxBoostrap}
    }

    describe("Deployment", async function () {
        it("should deploy fixture successfully", async function () {
            const {veSWPxBoostrap} = await deployFixture (); 
        })      
    })
})