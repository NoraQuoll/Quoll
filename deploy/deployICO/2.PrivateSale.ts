...
const deploy: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
  ...
  const startTime =    Math.floor(new Date("2024-03-20T00:00:00Z").getTime() / 1000);;
  const endTime = Math.floor(new Date("2024-03-23T23:59:59Z").getTime() / 1000);
  ...
  const data = await deploy("ICOPrivateSale", {
    from: deployer,
    args: [],
    log: true,
