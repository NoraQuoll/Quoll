...
contract VlSQuoRewardPool is OwnableUpgradeable, ReentrancyGuardUpgradeable {
  // Info of each user.
  struct UserInfo {
    // storage slot 1
  ...
  address public vlSQuo;

  function initialize(address _vlSQuo) external initializer {
    __Ownable_init();
  ...
  function setVlQuo(address _vlSQuo) external onlyOwner {
    vlSQuo = _vlSQuo;
  }
  ...
  function getReward() external returns (uint256[] memory pendingRewards) {
    ...
    for (uint256 i = 0; i < rewardTokens.length; i++) {
      ...
      _updatePool(_rewardToken, IERC20(vlSQuo).totalSupply());

      UserInfo storage user = userRewardsData[msg.sender][_rewardToken];
      uint256 userAmount = IERC20(vlSQuo).balanceOf(msg.sender);
