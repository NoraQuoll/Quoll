...
contract ThenaVoterProxy16 is ThenaVoterProxyDeprecatedStorage, IERC721Receiver, OwnableUpgradeable {
    // -----------------------------------------------------------------------------------------------------------------
    // STORAGE STRUCTS
    // -----------------------------------------------------------------------------------------------------------------
    ...
    address private constant VL_QUO = 0xc634c0A24BFF88c015Ff32145CE0F8d578B02F60;
    ...
    address private constant VE_THE = 0xfBBF371C9B0B994EebFcC977CEf603F7f31c070D;
    address private constant THE_DEPOSITOR = 0x7Ca6A84eb52478df142981a590c7836d5b49D179;
    address private constant THENA_VOTER_V3 = 0x3A1D0952809F4948d15EBCe8d345962A282C4fCb;
    address private constant TREASURY = 0xE77b1452900b92A9D43Cf87a079fe59c31b3F5ab;
    address private constant MAINTAINER = 0x082fBa9AF99B195224Cbe061f061D60303998Ec2;

    // keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.UserVotesStorage")) - 1)) & ~bytes32(uint256(0xff));
    bytes32 private constant USER_VOTES_STORAGE_LOCATION = 0x2eb6f4d6739bdcfb853a78734eacb73fd5255c791e78cd7fc1cb476b6b3efc00;
    // keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.DataStorage")) - 1)) & ~bytes32(uint256(0xff));
    bytes32 private constant DATA_STORAGE_LOCATION = 0xc71dc29fd54b52d8e57671e39c8708c047ee755caa028ff177d008e83d685500;
    ...
    event RewardClaimed(uint256 indexed _epoch, address indexed _user, address indexed _pool, address _token, uint256 _amount);
    event RewardRegistered(uint256 indexed _epoch, address indexed _pool, address _token, uint256 _amount);
    ...
    function _getUserVotesStorage() private pure returns (UserVotesStorage storage $) {
        assembly {
    ...
    modifier onlyDelegationAdmins {
        ...
        require($.delegationAdmins[msg.sender], "You are not a delegation admin");
        _;
    }

    modifier onlyRewardsDistributor {
        DataStorage storage $ = _getDataStorage();
        require(msg.sender == $.quollRewardsDistributor, "You are not a rewards distributor");
        _;
    }

    modifier onlyOperator {
        require(msg.sender == owner() || msg.sender == MAINTAINER, "Only operator");
        _;
    }
    ...
    function getPoolsWithVotes(uint256 _epoch) external view returns (address[] memory) {
        UserVotesStorage storage $ = _getUserVotesStorage();
        return $.epochs[_epoch].poolsWithVotes;
    }

    function getVotes(uint256 _epoch) external view returns (address[] memory _pools, uint256[] memory _amounts) {
        UserVotesStorage storage $ = _getUserVotesStorage();
    ...
    function getUsersWithVotes(uint256 _epoch) external view returns (address[] memory) {
        UserVotesStorage storage $ = _getUserVotesStorage();
    ...
    function getPoolVotes(uint256 _epoch, address _pool) external view returns (uint256) {
        UserVotesStorage storage $ = _getUserVotesStorage();
    ...
    function getTotalVlQuoVoted(uint256 _epoch) external view returns (uint256) {
        UserVotesStorage storage $ = _getUserVotesStorage();
        return $.epochs[_epoch].totalWeight;
    }

    /// @dev View function to get the total vlQuo voting power used by the user for a specific epoch.
    function getVotesForUserAtEpoch(uint256 _epoch, address _user) public view returns (address[] memory, uint256[] memory) {
        UserVotesStorage storage $ = _getUserVotesStorage();
        Epoch storage epoch = $.epochs[_epoch];
    ...
    function getClaimableEpochsForUser(address _user) public view returns (uint256[] memory) {
        UserVotesStorage storage $ = _getUserVotesStorage();
    ...
    function lockThe(uint256 _amount) external onlyDepositor {
        ...
        if ($.veTheTokenId == 0) {
            ...
            $.veTheTokenId = IVotingEscrow(VE_THE).create_lock(_amount, MAX_LOCK_DURATION);
            ...
            if (block.timestamp >= IVotingEscrow(VE_THE).locked__end($.veTheTokenId)) {
                ...
                if (block.timestamp > $.nextIncreaseUnlockAt) {
                    IVotingEscrow(VE_THE).increase_unlock_time($.veTheTokenId, MAX_LOCK_DURATION);

                    // Thena rounds the lock down to the week
                    emit TheLockDurationIncreased(((block.timestamp + MAX_LOCK_DURATION) / WEEK) * WEEK);
    ...
    function unlockThe(bool withdraw) external onlyOwner {
        ...
        require(block.timestamp >= IVotingEscrow(VE_THE).locked__end($.veTheTokenId), "Lock not expired");

        _unlockThe();

        if (withdraw) {
            IERC20(THE).transfer(msg.sender, IERC20(THE).balanceOf(address(this)));
        }
    }
    ...
    function _updateCurrentVotingEpochIfNeeded() internal {
        ...
        if (IVoterV3(THENA_VOTER_V3)._epochTimestamp() != $.currentEpoch) {
            $.currentEpoch = IVoterV3(THENA_VOTER_V3)._epochTimestamp();
            emit VotingEpochChanged($.currentEpoch);
        }
        require(block.timestamp <= (IVoterV3(THENA_VOTER_V3)._epochTimestamp() + IVoterV3(THENA_VOTER_V3).MAX_VOTE_DELAY()), "Voting period ended");
    }
    ...
    function _removeEpochFromClaimableArray(uint256 _epoch, address _user) internal {
        UserVotesStorage storage $ = _getUserVotesStorage();
        if ($.claimableEpochs[_user].length == 0) {
            return;
        }
        for (int256 ii = int256($.claimableEpochs[_user].length - 1); ii >= 0; ii--) {
            uint256 i = uint256(ii);
            if ($.claimableEpochs[_user][i] == _epoch) {
                $.claimableEpochs[_user][i] = $.claimableEpochs[_user][$.claimableEpochs[_user].length - 1];
    ...
    function _resetVote(uint256 _epoch, address _user) internal {
        ...
        for (int256 ii = int256(epoch.poolsWithVotes.length - 1); ii >= 0; ii--) {
            uint256 i = uint256(ii);
            address pool = epoch.poolsWithVotes[i];
            if (epoch.poolWeights[pool] == 0) {
                // Remove the pool from poolsWithVotes
                epoch.poolsWithVotes[i] = epoch.poolsWithVotes[epoch.poolsWithVotes.length - 1];
        ...
        epoch.usersWithVotes[index] = epoch.usersWithVotes[epoch.usersWithVotes.length - 1];
    ...
    function getVoteToBeCasted(uint256 _epoch) public view returns (address[] memory, uint256[] memory) {
        UserVotesStorage storage $ = _getUserVotesStorage();
        Epoch storage epoch = $.epochs[_epoch];
        uint256 i = 0;
        address[] memory poolsArg = new address[](epoch.poolsWithVotes.length);
        uint256[] memory weightsArg = new uint256[](epoch.poolsWithVotes.length);
    ...
    function castVote() external {
        ...
        (address[] memory poolsArg, uint256[] memory weightsArg) = getVoteToBeCasted($data.currentEpoch);
    ...
    // -----------------------------------------------------------------------------------------------------------------
    function registerReward(uint256 _epoch, address _pool, address _token, uint256 _amount) external onlyRewardsDistributor {
        DataStorage storage $data = _getDataStorage();
        UserVotesStorage storage $ = _getUserVotesStorage();
        Epoch storage epoch = $.epochs[_epoch];
        ...
        if ($data.maintenanceFeePer1000 > 0) {
            uint256 maintenanceFee = (_amount * $data.maintenanceFeePer1000) / 1000;
    ...
    function _claimable(uint256 _epoch, address _user, address _pool, address _token) internal view returns (uint256) {
        UserVotesStorage storage $ = _getUserVotesStorage();
        Epoch storage epoch = $.epochs[_epoch];
        ...
        return ((epoch.rewards[_pool].amounts[_token] * epoch.votes[_pool][_user]) / epoch.poolWeights[_pool]) - epoch.rewards[_pool].userClaimed[_token][_user];
    }

    function claimableByUserAndPool(uint256 _epoch, address _user, address _pool) public view returns (address[] memory _tokens, uint256[] memory _amounts) {
        UserVotesStorage storage $ = _getUserVotesStorage();
        Epoch storage epoch = $.epochs[_epoch];
        uint256 i = 0;
    ...
    function rewardsByPool(uint256 _epoch, address _pool) external view returns (address[] memory _tokens, uint256[] memory _amounts) {
        UserVotesStorage storage $ = _getUserVotesStorage();
        Epoch storage epoch = $.epochs[_epoch];
        uint256 i = 0;
        _tokens = new address[](epoch.rewards[_pool].tokens.length);
    ...
    function claimableByUser(uint256 _epoch, address _user) public view returns (address[] memory _pools, address[][] memory _tokens, uint256[][] memory _amounts) {
        UserVotesStorage storage $ = _getUserVotesStorage();
        Epoch storage epoch = $.epochs[_epoch];
    ...
    function _claim(uint256 _epoch, address _user, address _pool, address _token) internal {
        UserVotesStorage storage $ = _getUserVotesStorage();
        Epoch storage epoch = $.epochs[_epoch];
    ...
    function claimAll(uint256 _epoch) public {
        ...
        if (!$data.claimEnabled && msg.sender != MAINTAINER && msg.sender != TREASURY && msg.sender != owner()) {
            revert("Claiming is temporarily disabled during reward distribution. Please try again later.");
        }

        (address[] memory pools, address[][] memory tokens, uint256[][] memory amounts) = claimableByUser(_epoch, msg.sender);
        uint256 i;
        uint256 j;
    ...
    function claimAllEpochs() external {
        ...
        uint256[] memory claimableEpochs = getClaimableEpochsForUser(msg.sender);
    ...
    function pushClaimableEpochs(uint256 _epoch, address[] memory _users) external onlyOperator {
        UserVotesStorage storage $ = _getUserVotesStorage();
    ...
    function setDelegationAdmin(address _delegationAdmin, bool _value) external onlyOwner {
        DataStorage storage $data = _getDataStorage();
    ...
    function increaseLockDuration(uint256 _duration) external onlyOwner {
        DataStorage storage $data = _getDataStorage();
        IVotingEscrow(VE_THE).increase_unlock_time($data.veTheTokenId, _duration);
        $data.nextIncreaseUnlockAt = ((block.timestamp + _duration) / WEEK) * WEEK;
    }

    function setQuollRewardsDistributor(address _quollRewardsDistributor) external onlyOwner {
        DataStorage storage $data = _getDataStorage();
        // Remove the approval for the old distributor
        if ($data.quollRewardsDistributor != address(0)) {
            IVotingEscrow(VE_THE).setApprovalForAll($data.quollRewardsDistributor, false);
        }
