...
contract WombatBooster is IWombatBooster, OwnableUpgradeable {
  ...
  function setFees(
    ...
    uint256 total = _qWomIncentive.add(_vlQuoIncentive).add(_quoIncentive).add(
      _platformFee
    );
    ...
    require(_platformFee >= 0 && _platformFee <= 1000, "invalid _platformFee");
  ...
  function setEarmarkOnOperation(bool _earmarkOnOperation) external onlyOwner {
  ...
  function shutdownPool(uint256 _pid) public onlyOwner returns (bool) {
    ...
    if (pidToMasterWombat[_pid] == address(0)) {
      (rewardTokens, rewardAmounts) = IWombatVoterProxy(voterProxy).withdrawAll(
  ...
  function deposit(uint256 _pid, uint256 _amount, bool _stake) public override {
    ...
    uint256[] memory rewardAmounts;
    if (pidToMasterWombat[_pid] == address(0)) {
      (rewardTokens, rewardAmounts) = IWombatVoterProxy(voterProxy).deposit(
        pool.masterWombatPid,
        _amount
      );
    } else {
      (rewardTokens, rewardAmounts) = IWombatVoterProxy(voterProxy).depositV2(
  ...
  function _withdraw(
    ...
    if (!pool.shutdown) {
      ...
      uint256[] memory rewardAmounts;
      if (pidToMasterWombat[_pid] == address(0)) {
        (rewardTokens, rewardAmounts) = IWombatVoterProxy(voterProxy).withdraw(
          pool.masterWombatPid,
          _amount
        );
      } else {
        (rewardTokens, rewardAmounts) = IWombatVoterProxy(voterProxy)
          .withdrawV2(pidToMasterWombat[_pid], pool.masterWombatPid, _amount);
      }
      // rewards are claimed when withdrawing
      _updatePendingRewards(_pid, rewardTokens, rewardAmounts);
  ...
  function _earmarkRewards(uint256 _pid, address _caller) internal {
    ...
    if (womBal > 0) {
      ...
      if (platformFee > 0) {
        ...
        uint256 _platform = womBal.mul(platformFee).div(FEE_DENOMINATOR);
      ...
      if (qWomIncentiveAmount > 0) {
        ...
        IRewards(qWomRewardPool).queueNewRewards(wom, qWomIncentiveAmount);
      }
