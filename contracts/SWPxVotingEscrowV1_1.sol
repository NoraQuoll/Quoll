...
import "./Interfaces/VeSWPx/token/ERC721/extensions/IERC721Metadata.sol";
import "./Interfaces/VeSWPx/IVotes.sol";
import "./Interfaces/VeSWPx/proxy/utils/Initializable.sol";
import "./Interfaces/VeSWPx/token/ERC721/IERC721Receiver.sol";
import  "./Interfaces/VeSWPx/IERC20.sol";
import  "./Interfaces/VeSWPx/IVeArtProxyV1_1.sol";
import  "./Interfaces/VeSWPx/IVotingEscrow.sol";
...
contract VotingEscrowV1_1 is Initializable, IERC721, IERC721Metadata, IVotes {
    enum DepositType {
        DEPOSIT_FOR_TYPE,
        CREATE_LOCK_TYPE,
    ...
    address public constant token = 0x1bE1008A72433fd70718411211e49394Cd05Fc23; // for testnet deployment
    ...
    string public constant name = "veSwapX";
    string public constant symbol = "veSWPx";
    ...
    function tokenURI(uint256 _tokenId) external view returns (string memory) {
        ...
        bool isListable = attachments[_tokenId] == 0 && !voted[_tokenId];
        return IVeArtProxyV1_1(artProxy)._tokenURI(
            _tokenId, _balanceOfNFT(_tokenId, block.timestamp), _locked.end, uint256(int256(_locked.amount)), isListable
        );
    }
    ...
    event DepositCheckpoint(address user, uint256 tokenId, int128 bias, int128 slope, uint256 ts, uint256 blk);
    ...
    function locked__start(uint256 _tokenId) external view returns (uint256) {
    ...
    function _checkpoint(uint256 _tokenId, LockedBalance memory old_locked, LockedBalance memory new_locked) internal {
        ...
        {
            uint256 t_i = (last_checkpoint / Constants.EPOCH_LENGTH) * Constants.EPOCH_LENGTH;
    ...
    function deposit_for(uint256 _tokenId, uint256 _value) external nonreentrant returns (uint256 votingPower) {
        LockedBalance memory _locked = locked[_tokenId];
    ...
    function _create_lock(uint256 _value, uint256 _lock_duration, address _to) internal returns (uint256) {
        uint256 unlock_time = (block.timestamp + _lock_duration) / Constants.EPOCH_LENGTH * Constants.EPOCH_LENGTH; // Locktime is rounded down to weeks
    ...
    function create_lock(uint256 _value, uint256 _lock_duration)
        external
        nonreentrant
        returns (uint256 newTokenId, uint256 votingPower)
    {
    ...
    function increase_amount(uint256 _tokenId, uint256 _value) external nonreentrant returns (uint256 votingPower) {
    ...
    function _supply_at(Point memory point, uint256 t) internal view returns (uint256) {
        ...
        uint256 t_i = (last_point.ts / Constants.EPOCH_LENGTH) * Constants.EPOCH_LENGTH;
        for (uint256 i = 0; i < 255; ++i) {
            t_i += Constants.EPOCH_LENGTH;
            int128 d_slope = 0;
            if (t_i > t) {
     ...
     */
    function split(uint256[] memory amounts, uint256 _tokenId)
        external
        returns (uint256[] memory newTokenIds, uint256[] memory votingPowers)
    {
        ...
        // save end
        uint256 unlock_time = _locked.end;
        require(unlock_time > block.timestamp, "Can only lock until time in the future");
        require(unlock_time <= block.timestamp + MAXTIME, "Voting lock can be 2 years max");
