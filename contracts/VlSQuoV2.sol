...
import "./Interfaces/IVlSQuoV2.sol";
import "./Interfaces/ISwapxVoterProxy.sol";
import "./VlSQuoRewardPool.sol";


contract VlSQuoV2 is
    IVlSQuoV2,
    OwnableUpgradeable,
    ...
    PausableUpgradeable
{
    ...
    IERC20 public override squo;

    ISwapxVoterProxy public swapxVoterProxy;
    ...
    function lock(
        ...
        squo.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function unlock(uint256 _slot) external nonReentrant whenNotPaused {
        ...
        if (punishment > 0) {
            squo.safeTransfer(treasury, punishment);
        }
        squo.safeTransfer(msg.sender, lockInfo.quoAmount.sub(punishment));
    ...
    function _increaseBalance(
        ...
        VlSQuoRewardPool(rewardPool).updateBalVlquoV2(_user, _balances[_user], _balances[_user].add(_amount), _totalSupply);
    ...
    function _decreaseBalance(
        ...
        VlSQuoRewardPool(rewardPool).updateBalVlquoV2(_user, _balances[_user], _balances[_user].sub(_amount), _totalSupply);
    ...
    function getCurrentVoteForUser(address _user) public view returns(uint256) {
        (address[] memory pools, uint256[] memory userVotes) = swapxVoterProxy.getCurrentVotesForUser(_user);
        uint256 totalVote = 0;
        for(uint i = 0; i < pools.length ; i++){
            totalVote+= userVotes[i]; //user vote weight for each pool
        }
        return totalVote;
    }
}
