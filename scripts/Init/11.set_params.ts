...
async function main() {
    ...
    const zap = await getContracts()[process.env.NETWORK_NAME!][
        "QuollZap"
      ]["address"];
  ...
  const QuollZap = JSON.parse(
    fs.readFileSync(
      "./artifacts/contracts/QuollZap.sol/QuollZap.json",
      "utf-8"
    )
  ).abi;
  ...
  const contract = new web3.eth.Contract(QuollZap);

  const txData = contract.methods
    .setParams(
      process.env.WNATIVE,
      process.env.WNATIVE_RELAYER,
      booster,
      nativeZap,
      process.env.USDT,
      quo
