...
const rewardPool = "0xcCac06A4ee305050f84578F60DCd39935696C36A";
...
async function setParamsBoostrap() {
  ...
  const txData = contract.methods
    .setParams(qCake, cake, depositor, campaignLens)
    .encodeABI();
...
async function setParamsDepositor() {
  ...
  const txData = contract.methods
    .setParams(cake, voterProxy, qCake, rewardPool)
    .encodeABI();
...
async function setParamsVoterProxy() {
  ...
  const txData = contract.methods
    .setParams(masterChef, cake, veCake, booster, depositor)
    .encodeABI();
